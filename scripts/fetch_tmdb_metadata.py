...
# Gli episodi vivono fuori da metadata.js: sono i tre quarti del peso e
# servono solo a chi apre il pannello di una serie. Un file per serie,
# scaricato al primo clic.
EPISODES_DIR = os.path.join(ROOT, "data", "episodes")
...
def load_previous_doc():
    """Il documento committato per intero, o {} se non c'è / è illeggibile."""
    try:
...
def load_previous():
    ...
    return load_previous_doc().get("items") or {}


def write_if_changed(path, text):
    """Scrive solo se il contenuto cambia davvero.

    Il workflow committa quando git vede una differenza: riscrivere file
    identici produrrebbe commit vuoti di sostanza e farebbe ripartire il
    deploy per niente.
    """
    try:
...
def detach_episodes(result_items):
    """Stacca gli array `episodes` dalle voci, lasciando i due totali.

    Al posto dell'elenco restano `episodeCount` e `episodeHours`: bastano per
    il badge "n/m ep." e per le ore totali, che è tutto ciò che serve prima
    che qualcuno apra il pannello di quella serie.

    Le voci conservate da un run precedente non hanno l'array (è già stato
    staccato allora) e vengono lasciate stare: il loro file su disco è ancora
    quello giusto.
    """
    detached = {}
    for item_id, entry in list(result_items.items()):
        episodes = entry.get("episodes")
        if episodes is None:
            continue
        slim = dict(entry)
        slim.pop("episodes")
        slim["episodeCount"] = len(episodes)
        slim["episodeHours"] = round(sum((e.get("runtime") or 0) for e in episodes) / 60, 3)
        result_items[item_id] = slim
        detached[item_id] = episodes
    return detached


def write_episode_files(detached, result_items):
    """Un file per serie, più la rimozione di quelli rimasti orfani."""
    os.makedirs(EPISODES_DIR, exist_ok=True)
    written = 0
    for item_id, episodes in detached.items():
        text = json.dumps(episodes, ensure_ascii=False, indent=1) + "\n"
        if write_if_changed(os.path.join(EPISODES_DIR, f"{item_id}.json"), text):
            written += 1
    ...
    for name in os.listdir(EPISODES_DIR):
        if not name.endswith(".json"):
            continue
        item_id = name[:-5]
        entry = result_items.get(item_id)
        # Resta solo ciò che il tracker elenca ancora e che ha davvero episodi.
        if entry and entry.get("episodeCount"):
            continue
        os.remove(os.path.join(EPISODES_DIR, name))
        removed += 1
...
def main():
    ...
    detached = detach_episodes(result_items)
    ep_written, ep_removed = write_episode_files(detached, result_items)

    # generatedAt cambia a ogni esecuzione e da solo basterebbe a produrre una
    # differenza: si aggiorna solo quando cambia qualcosa di sostanziale,
    # altrimenti il file resta identico e non c'è niente da committare.
    body = {"region": REGION, "language": LANG, "items": result_items}
    previous_doc = load_previous_doc()
    unchanged = all(previous_doc.get(k) == v for k, v in body.items())
    generated_at = (previous_doc.get("generatedAt") if unchanged
                    else time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()))

    output = dict(body)
    output["generatedAt"] = generated_at or time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    output = {k: output[k] for k in ("generatedAt", "region", "language", "items")}

    meta_written = write_if_changed(
        OUTPUT_PATH,
        "/* Generated by scripts/fetch_tmdb_metadata.py — do not edit by hand. */\n"
        "const TMDB_METADATA = " + json.dumps(output, ensure_ascii=False, indent=1) + ";\n")

    print(f"metadata.js {'riscritto' if meta_written else 'invariato'}; "
          f"episodi: {ep_written} file aggiornati, {ep_removed} rimossi, "
          f"{len(detached)} serie con elenco")
